...
if(isset($_POST['insert_costumer'])) {
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        if (empty($errors)) {
            // Initialize initial_amt with the same value as cos_amount
            $initial_amt = $cos_amount;
            $insert_cos = "INSERT INTO costumers (cos_name, mobile, village, p_status, cos_address, cos_amount, initial_amt, payment_due) 
                           VALUES ('$costumer_name', '$mob', '$village', '$status', '$cos_add', '$cos_amount', '$initial_amt', '$payment_due')";
            $run_cos = mysqli_query($con, $insert_cos);
            if($run_cos){
                echo "<script>alert('Supplier inserted successfully')</script>";
            } else {
                echo "<script>alert('Error inserting Supplier')</script>";
            }
        } else {
