...
CASE
    WHEN f.factory_amount > (SELECT SUM(amount) FROM factory_transactions WHERE factory_id = f.factory_id AND transaction_type = 'add') - (SELECT SUM(amount) FROM factory_transactions WHERE factory_id = f.factory_id AND transaction_type = 'subtract') THEN 'Deposit'
    ELSE 'Payout'
END AS transaction_type,
ABS(f.factory_amount - ((SELECT SUM(amount) FROM factory_transactions WHERE factory_id = f.factory_id AND transaction_type = 'add') - (SELECT SUM(amount) FROM factory_transactions WHERE factory_id = f.factory_id AND transaction_type = 'subtract'))) AS amount,
...
<body>
    <div class="container">
        ...
        <p><strong>Amount Due:</strong> <?php echo $factory_data['factory_amount']; ?></p>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if (isset($row_transactions['transaction_id'])) { ?>
                                <a href="generate_bill_factory_transaction.php?factory_id=<?php echo $factory_id; ?>&transaction_id=<?php echo $row_transactions['transaction_id']; ?>">Generate Bill</a>
                            <?php } ?>
                        </td>
                    </tr>
                <?php } ?>
